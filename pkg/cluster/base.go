...
const (
	...
	dockerInstallScript = "https://get.docker.com | bash -s docker --mirror Aliyun"
)
...
func (p *ProviderBase) JoinNodes(cloudInstanceFunc func(ssh *types.SSH) (*types.Cluster, error),
	...
	if syncExistInstance != nil {
		...
		if err != nil {
			p.Logger.Errorf("[%s] failed to sync existing instance instance, got error %v", p.Provider, err)
			return err
		}
	}
