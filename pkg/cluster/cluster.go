...
// DeleteNode cordons and drains a node, add by edgego
func DeleteNode(nodeName string, client *kubernetes.Clientset, DrainWaitSeconds int) error {
	if client == nil {
	...
	if err != nil {
		_, _ = fmt.Fprintf(os.Stderr, "%v\n", err)
		return fmt.Errorf("delete node: [%s] error : [%s]", nodeName, err.Error())
	}
