...
// DeleteK3sNode delete K3s node add by edgego
func (p *Native) DeleteK3sNode(clusterID, nodeName, instanceId string) error {
	...
	p.Logger.Infof("[%s] begin to delete cluster node %v...", p.Provider, nodeName)
	...
	masterNum, _ := strconv.Atoi(state.Master)
	workerNum, _ := strconv.Atoi(state.Worker)
	// if only one master node , not delete it
	if masterNum > 1 {
		masterNodes := make([]types.Node, 0)
		err = json.Unmarshal(state.MasterNodes, &masterNodes)
		if err != nil {
			_, _ = fmt.Fprintf(os.Stderr, "failed to Unmarshal master nodes, error %s\n", err.Error())
			return fmt.Errorf("failed to Unmarshal master nodes, error %s\n", err.Error())
		}

		for index, node := range masterNodes {
			if node.InstanceID == instanceId {
				masterNodes = append(masterNodes[:index], masterNodes[index+1:]...)

				masterBytes, err := json.Marshal(masterNodes)
				if err != nil {
					_, _ = fmt.Fprintf(os.Stderr, "failed to marshal master nodes, error %s\n", err.Error())
					return fmt.Errorf("failed to marshal master nodes, error %s\n", err.Error())
				}

				state.Status = common.StatusUpdating
				...
				state.Master = strconv.Itoa(masterNum - 1)
	...
	if workerNum > 0 {
		...
		if err != nil {
			_, _ = fmt.Fprintf(os.Stderr, "failed to Unmarshal worker nodes, error %s\n", err.Error())
			return fmt.Errorf("failed to Unmarshal worker nodes, error %s\n", err.Error())
		}
		...
		for index, node := range workerNodes {
			if node.InstanceID == instanceId {
				...
				workerBytes, err := json.Marshal(workerNodes)
				if err != nil {
					_, _ = fmt.Fprintf(os.Stderr, "failed to marshal worker nodes, error %s\n", err.Error())
					return fmt.Errorf("failed to marshal worker nodes, error %s\n", err.Error())
				}
				...
				state.Worker = strconv.Itoa(workerNum - 1)
